...
if ($_GET['theme']) {
	if ($_GET['theme'] == "default") {
		set_cookie("theme", "", 86400 * 1);
		goto_url($_SERVER['SCRIPT_NAME']);
		exit;
	} else {
		set_cookie("theme", $_GET['theme'], 86400 * 1);
		goto_url(G5_URL);
		exit;
	}
}
//echo get_cookie('theme');
...
<ul id="theme_list">
	...
	for($i=0; $i<$total_count; $i++) {
		...
		if(get_cookie("theme") == $theme[$i]) {
			$btn_cookie = '<span class="theme_sl theme_sl_use">현재쿠키</span>';
			$btn_cookie .= '<button type="button" class="theme_sl" style="margin-left:4px;" onClick="location.href=\'' . $_SERVER['SCRIPT_NAME'] . '?theme=default\'">쿠키삭제</button>';
		} else {
			$btn_cookie = '';
		}
	?>
	<li>
		...
		<?php //echo $btn_active; ?>
		<?php echo $btn_cookie; ?>
		<a href="<?php echo $_SERVER['SCRIPT_NAME']; ?>?theme=<?php echo $theme[$i]; ?>" class="theme_pr" target="theme_preview">미리보기쿠키설정</a>
